...
rec <- rec %>% 
  ...
  step_mutate_at(starts_with("automated_underwriting_system"), fn = function(x) if_else(is.na(x), 6, x)) %>%
  # imputation of numeric variables
  step_impute_linear(income, combined_loan_to_value_ratio, property_value) %>%
  ...
  step_mutate_at(loan_type:business_or_commercial_purpose, fn = function(x) as.factor(x)) %>% 
  # remove zero variance variables
  step_zv(loan_type:business_or_commercial_purpose) %>%
  # dummy encoding of nominal variables
  step_dummy(loan_type:business_or_commercial_purpose)

# test recipe
test <- suppressWarnings(rec %>% prep() %>% bake(new_data = NULL))
