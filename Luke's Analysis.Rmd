...
lab_income_plot <- lab_income_train %>% filter(posIncome == TRUE) %>% ggplot(aes(x = income)) + geom_histogram(bins = 20)
...
colnames()
train_split <- train %>% vfold_cv(v = 10, strata = c("action_taken",))
help(vfold_cv)
