...
```{r RUN THIS: training and test data change colsToRemove larger}

# makes it easier to implement model

colsToRemove <- c("id","activity_year", "legal_entity_identifier_lei", "total_points_and_fees", "co_applicant_or_co_borrower_name_and_version_of_credit_scoring_model", "ethnicity_of_applicant_or_borrower_3", "ethnicity_of_applicant_or_borrower_4", "ethnicity_of_applicant_or_borrower_5", "ethnicity_of_co_applicant_or_co_borrower_3", "ethnicity_of_co_applicant_or_co_borrower_4", "ethnicity_of_co_applicant_or_co_borrower_5", "race_of_applicant_or_borrower_3", "race_of_applicant_or_borrower_4", "race_of_applicant_or_borrower_5", "race_of_co_applicant_or_co_borrower_4", "race_of_co_applicant_or_co_borrower_5")

# TRAINING DATA

# remove missing rows based on this variable ****NEW*** Eric's Addition
# train <- train %>% drop_na(age_of_co_applicant_62)
...
```{r RUN THIS: training and test data change -- Luke's colsToRemove}
...
```{r Eric's Subsampling}
set.seed(10)
data_split <- initial_split(train, prop = .1, strata = action_taken)

train_sample <- training(data_split)
train_test_sample <- testing(data_split)
train_sample$action_taken <- as.factor(train_sample$action_taken)

train_sample_folds <- vfold_cv(train_sample, v=10, strata = action_taken)
```
...
nn_model <- mlp(hidden_units = tune(), epochs = 500, learn_rate = tune(), activation = "relu") %>% set_engine("brulee") %>% set_mode("classification") %>% translate()

bayes_model <- naive_Bayes(smoothness = tune()) %>% set_engine("klaR") %>% set_mode("classification")

model_list <- list(decision_tree_model = decision_tree_model, boost_tree_model = boost_tree_model, bayes_model = bayes_model, nn_model = nn_model, log_model = log_model)
...
# Based on Eric's subsampling
rec1 <- recipe(action_taken ~ ., data = train_sample)
...
wf_set_res <- wf_set %>% workflow_map(fn = "tune_grid", resamples = train_sample_folds, grid = 5, verbose = TRUE, metrics = wf_metrics)

autoplot(wf_set_res)
wf_set_res %>% collect_metrics() %>% filter(.metric == "f_meas") %>% arrange(desc(mean))
```

```{r boosted tree tune on subsample -- non ommitted rows}
### Boosted Tree -- Optimally tuned at: learn rate =  )
set.seed(10)
wf_bt <- workflow() %>% add_model(boost_tree_model) %>% add_recipe(rec)

recipes_param <- extract_parameter_set_dials(boost_tree_model)

grid_bt <- grid_regular(
  learn_rate(range = c(.0008, .0012),
             trans = NULL),
  levels = 5)

wf_bt_res <- wf_bt %>% tune_grid(resamples = train_sample_folds, grid = grid_bt, metrics = wf_metrics)

autoplot(wf_bt_res)

wf_bt_res %>% collect_metrics() %>% filter(.metric == "f_meas") %>% arrange(desc(mean))
```



```{r boosted tree tune testing f measure on training-test}
final_wf_param <- wf_bt_res %>% select_best(metric = "f_meas")

final_model <- finalize_workflow(wf_bt, final_wf_param)

final_model_fit <- final_model %>% fit(data = train_sample)

preds_sample <- final_model_fit %>% predict(new_data = train_test_sample)

training_pred_chart <- train_test_sample %>% dplyr:::select(action_taken) %>% bind_cols(preds_sample)

f_meas(truth = action_taken, estimate = .pred_class, data = training_pred_chart)
```


```{r boosted tree tune printing results on full train on test}
final_wf_param <- wf_bt_res %>% select_best(metric = "f_meas")
...
test <- read.csv(testFilepath)
test_preds <- test %>% dplyr::select(id) %>% bind_cols(preds) %>% rename(action_taken = .pred_class)

# export predictions
write_csv(test_preds, "boostedpred3.csv")
```
...
train_sample_split <- vfold_cv(train_sample, v=10, strata = action_taken)
...
log_fit <- log_wf %>% fit(data = train_sample)

log_cl_res <- log_wf %>%
  fit_resamples(
    resamples = train_sample_split,
    metrics = wf_metrics,
  )

log_cl_res %>% collect_metrics()


log_fitted <- log_fit %>% predict(new_data = train_sample)

sample_results <- log_fitted %>% bind_cols(train_sample$action_taken)

f_meas(sample_results, truth = ...2, estimate = .pred_class)

log_preds <- log_fit %>% predict(new_data = test)

test <- read.csv(testFilepath)

test_results <- test %>% dplyr::select(id) %>% bind_cols(log_preds) %>% dplyr::rename(action_taken = .pred_class)

write_csv(test_results, "log_test0.csv")

```

```{r Mayerli's plots}
plot1 <- ggplot(train, aes(x = as.factor(action_taken), y = loan_amount)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Loan Amount by Action Taken", x = "Action Taken", y = " Loan Amount") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

plot2 <- ggplot(train, aes(x = as.factor(action_taken), y = property_value)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Property Value by Action Taken", x = "Action Taken", y = "Property Value") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

plot3 <- ggplot(train, aes(x = as.factor(action_taken), y = income)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Income by Action Taken", x = "Action Taken", y = "Income") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

plot4 <- ggplot(train, aes(x = as.factor(action_taken), y = combined_loan_to_value_ratio)) +
  geom_bar(stat = "identity", fill = "steelblue") +
